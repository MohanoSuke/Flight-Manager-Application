...
def indice_par_dichotomie(tab,tab_indice ,val,cpt):
    ...
    sortie m = int
    post-cond : Lorsque val est dans tab, on a tab[m] = val, dans le cas contraire, m vaut -1
    """
    m=-1
    d=0
    f=len(tab)-1
...
def age( date,cpt):      #on definit la date actuelle comme le 12/01/2024
    """"
    entrée date : int
    sortie age : int
    pre-condition : 1 <= (date //1000000)%100 <= 31 ; 1 <= (date //10000)%100 <= 12 ; 1900 <= date %10000 <= 2024
    post-condition : age est l'age correspondant à la date de naissance donnéee par date.
    """
    jour= (date //1000000)%100
    mois = (date //10000)%100
    annee = date %10000
    cpt += 8
...
def indice_par_dichotomie_vol(tab,tab_indice ,heure,cpt):
    ...
    pré-cond : ∀ i ∈ [1, len(tab) [, tab[i-1] < tab[i]
    sortie m = int
    post-cond : Lorsque val est dans tab, on a tab[m] = heure, dans le cas contraire, on a tab[m], la valeur la plus proche de m
    """
    d=0
    f=len(tab)-1
    cpt += 3
...
affichage_vol(tab3, 1500)  #on rentre l'heure ici
...
reprogrammation_vol(tab4, 2)  #on donne ici l'indice du vol qu'on veut reprogrammer
